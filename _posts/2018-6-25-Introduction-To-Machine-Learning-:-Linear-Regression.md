...
Then if we let $$y_n$$ denote our answer, or *'prediction'*, and $t_{n}$ denote the true answer, which is available in the training data at training time, then let us call $y_n - t_n$ be called the **'error'**.
