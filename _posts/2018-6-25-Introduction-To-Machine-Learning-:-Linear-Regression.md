...
Then if we let $\alpha$ denote our answer, or *'prediction'*, and $t_n$ denote the true answer, which is available in the training data at training time, then let us call $y_n - t_n$ be called the **'error'**.
